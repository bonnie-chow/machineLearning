...
reduced_train_set = column_selector.consider_subset_of_columns(train_without_id,
                                                               ['budget', 'genres', 'original_language', 'tagline',
                                                                'popularity', 'release_date', 'runtime', 'Keywords',
                                                                'overview', 'cast', 'crew', 'revenue'])
...
reduced_train_set['main_genre'] = str(
    dictionary_data_extractor.extract_data_from_dict(reduced_train_set, 'genres', 'name'))
reduced_train_set['main_actor'] = str(
    dictionary_data_extractor.extract_data_from_dict(reduced_train_set, 'cast', 'name'))
reduced_train_set['main_actor'] = str(
    dictionary_data_extractor.convert_if_condition_is_met(reduced_train_set, 'main_actor', 3))
reduced_train_set['main_keyword'] = str(
    dictionary_data_extractor.extract_data_from_dict(reduced_train_set, 'Keywords', 'name'))
reduced_train_set['main_keyword'] = str(
    dictionary_data_extractor.convert_if_condition_is_met(reduced_train_set, 'main_keyword', 6))
reduced_train_set['main_crew_member'] = str(
    dictionary_data_extractor.extract_data_from_dict(reduced_train_set, 'crew', 'name'))
reduced_train_set['main_crew_member'] = str(
    dictionary_data_extractor.convert_if_condition_is_met(reduced_train_set, 'main_crew_member', 6))


tagline_embeddings, overview_embeddings = column_data_preprocessor.map_embeddings_df_columns(reduced_train_set, 'tagline', 'overview')

for i in range(0, 50):
    reduced_train_set['tagline ' + str(i + 1)] = pd.DataFrame({'tagline ' + str(i + 1): np.array(tagline_embeddings)[:, i]})

for i in range(0, 50):
    reduced_train_set['overview ' + str(i + 1)] = pd.DataFrame({'overview ' + str(i + 1): np.array(overview_embeddings)[:, i]})

train_after_handling_dicts = column_selector.remove_unwanted_columns(reduced_train_set,
                                                                     ['original_language', 'genres', 'tagline', 'Keywords', 'cast',
                                                                      'overview', 'crew', 'release_date'])
...
for i in range(0, 50):
    num_attributes.append('tagline ' + str(i + 1))
for i in range(0, 50):
    num_attributes.append('overview ' + str(i + 1))
...
# param_grid = [
#     {'n_estimators': [2], 'max_depth': [30, 40, 50], 'max_features': [40, 50, 60]}
# ]
#
...
model.fit(X_train, y_train,
          batch_size=30, epochs=30, validation_data=(X_val, y_val), callbacks=[learning_rate_reduction])
print('model succesfully trained')
predictions = model.predict(X_val)

relative_validation_errors, rmsle_validation = model_evaluator.evaluate_model(model, X_val, y_val, 40)
relative_train_errors, rmsle_train = model_evaluator.evaluate_model(model, X_train, y_train, 40)
print('relative validation errors:')
print('validation:', relative_validation_errors)
print('train', relative_train_errors)
print('rmsle error of the model:')
